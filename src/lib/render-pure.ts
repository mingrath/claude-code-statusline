...
function formatWeeklyPart(
	...
	// For weekly cost: sevenDay.utilization holds the weekly total when from ccusage
	// periodCost is the 5-hour block cost, so prefer sevenDay.utilization for weekly display
	const weeklyCost =
		sevenDay.utilization > periodCost ? sevenDay.utilization : periodCost;
	if (config.cost.enabled && weeklyCost > 0) {
		parts.push(
			`${colors.gray("$")}${colors.dimWhite(formatCost(weeklyCost, config.cost.format))}`,
		);
	}
