...
async function main() {
	try {
		...
		// periodCost is only relevant when spend tracking is enabled
		...
		const data: StatuslineData = {
			...
			contextTokens,
			contextPercentage,
			...((getUsageLimits || usageLimits.five_hour || usageLimits.seven_day) && {
				usageLimits: {
			...
			...((getPeriodCost || getTodayCostV2 || periodCost !== undefined) && { periodCost, todayCost }),
		};
